###############################################################################
# Table 2 Replication — Heterogeneous-Variance + Cross-Cohort DGP (GMM_Eff_HetCov design)
#
# DGP: Y_it = alpha_i + lambda_t + tau_it + epsilon_it
#   epsilon_it = lambda_i * f_t + u_it
#
#   f_t  : common AR(1) factor  (rho_f, sigma_f)
#   lambda_i : unit factor loading ~ N(lambda_mean_g, lambda_sd), varies by cohort
#   u_it : idiosyncratic AR(1) error, cohort-specific (rho_idio_g, sigma2_idio_g),
#          cross-cohort independent
#
# Implication:
#   Within-cohort Cov(eps_it, eps_it') = lambda_mean_g^2 * Cov(f_t,f_t') + sigma2_g*rho_g^|t-t'|
#   Cross-cohort  Cov(eps_it, eps_jt') = lambda_mean_g * lambda_mean_g' * Cov(f_t,f_t')  (!=0)
#
#   => Assumption 1 violated; GMM_Eff_HetCov is the efficient estimator.
#
# Estimators:
#   1. TWFE
...
# Cohort ordering used in DGP loops (g=10, 13, 16, 19, 22, 0=never-treated)
cohort_dgp_order <- c(treatment_times, 0L)

# Idiosyncratic within-cohort AR(1) — cohort-specific, cross-cohort independent
rho_idio_by_cohort    <- c(0.2, 0.5, 0.7, 0.9, 0.4, 0.6)
sigma2_idio_by_cohort <- c(0.5, 1.0, 2.0, 3.0, 1.5, 1.0)
Sigma_idio_by_cohort  <- lapply(seq_along(cohort_dgp_order), function(gi)
  sigma2_idio_by_cohort[gi] * toeplitz(rho_idio_by_cohort[gi]^(0:(T_total - 1))))

# Common factor parameters (induces cross-cohort dependence)
rho_f                 <- 0.5     # AR(1) persistence of common factor
sigma_f               <- 1.0    # innovation std dev of common factor
lambda_mean_by_cohort <- c(0.8, 1.2, -0.5, 1.5, 0.3, 1.0)  # cohort mean loadings
lambda_sd             <- 0.1    # within-cohort dispersion of loadings
...
generate_data <- function(beta_g_vec, r_g_vec) {
  ...
  alpha    <- rnorm(N_total)
  lambda_t <- rnorm(T_total)   # time FE (renamed to avoid clash with factor loadings)

  # Common factor: stationary AR(1) initialisation
  f_common    <- numeric(T_total)
  f_common[1] <- rnorm(1, sd = sigma_f / sqrt(1 - rho_f^2))
  for (tt in 2:T_total)
    f_common[tt] <- rho_f * f_common[tt - 1] + rnorm(1, sd = sigma_f)

  # Unit factor loadings — cohort mean varies, within-cohort dispersion small
  lambda_i <- numeric(N_total)
  for (gi in seq_along(cohort_dgp_order)) {
    g_val   <- cohort_dgp_order[gi]
    units_g <- which(unit_cohort == g_val)
    lambda_i[units_g] <- rnorm(length(units_g),
                               mean = lambda_mean_by_cohort[gi], sd = lambda_sd)
  }

  # Idiosyncratic errors: cohort-specific AR(1), independent across cohorts
  eps_idio <- matrix(0.0, nrow = N_total, ncol = T_total)
  for (gi in seq_along(cohort_dgp_order)) {
    g_val   <- cohort_dgp_order[gi]
    units_g <- which(unit_cohort == g_val)
    if (length(units_g) == 0L) next
    eps_idio[units_g, ] <- mvrnorm(length(units_g), rep(0, T_total),
                                   Sigma_idio_by_cohort[[gi]])
  }

  # Total error: factor component (cross-cohort dependent) + idiosyncratic
  eps_mat <- outer(lambda_i, f_common) + eps_idio   # N_total x T_total
  eps     <- as.vector(t(eps_mat))
  ...
  dt <- data.table(unit = unit_id, time = time_id,
                   Y = alpha[unit_id] + lambda_t[time_id] + tau_vec + eps,
                   D = D_vec, g = g_vec)
...
cat("================================================================\n")
cat(sprintf("  N=%d (%d/cohort), T=%d, %d sims, 10 estimators\n",
            N_total, cohort_size, T_total, n_sims))
cat(sprintf("  Factor DGP: rho_f=%.1f, sigma_f=%.1f\n", rho_f, sigma_f))
cat(sprintf("  lambda_mean: %s\n", paste(round(lambda_mean_by_cohort, 2), collapse = ", ")))
cat(sprintf("  rho_idio:   %s\n", paste(round(rho_idio_by_cohort, 2), collapse = ", ")))
cat(sprintf("  sigma2_idio: %s\n", paste(round(sigma2_idio_by_cohort, 2), collapse = ", ")))
...
cat("\n\n")
cat("==========================================================================\n")
cat(sprintf("  TABLE 2: Bias and Variance — Factor DGP (cross-cohort dependent), N=%d (%d/cohort)\n",
            N_total, cohort_size))
cat(sprintf("  f~AR(%.1f), lambda_mean: %s\n",
            rho_f, paste(round(lambda_mean_by_cohort, 2), collapse = ",")))
...
save(res_hom, res_het, table2,
     ...
     file = "simulation_dependent_errors_no_fc3_results.RData")
cat("\nResults (incl. CATTs) saved to simulation_dependent_errors_no_fc3_results.RData\n")
