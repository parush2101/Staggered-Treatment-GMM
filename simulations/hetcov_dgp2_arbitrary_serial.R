# GMM I (Toeplitz homosk.) + GMM II (Toeplitz heterosk.) + GMM III (HetCov) | DGP2
# ARBITRARY within-unit serial correlation (NOT AR(1)): a general stationary
# covariance built as a positive mixture of AR kernels, swept over three regimes
#   serial in {low, medium, high}   cohort_size=50  n_never=50  n_sims=500 | HETEROGENEOUS
...
serial_types <- c("low", "medium", "high")   # arbitrary serial-correlation regimes
Sigma_err    <- NULL                          # set per regime inside the loop below


# ----------------------------------------------------------------------------
# Arbitrary within-unit serial correlation.
#
# Rather than a single AR(1), the errors follow a *general* stationary covariance
# built as a positively-weighted mixture of AR(1) kernels. Each Toeplitz(rho^|k|)
# is positive-definite for |rho| < 1, so any positive mixture is PD; a small
# negative-rho component injects sign changes into the autocorrelation function,
# so the implied ACF is NOT geometric -- it is an arbitrary, non-AR(1) serial-
# correlation structure with two decay timescales plus mild oscillation. The
# marginal variance is normalised to 1, so the three regimes differ only in the
# STRENGTH of serial dependence, not in scale:
#
#   "low"    weak, short memory      (lag-1 autocorr ~ 0.19)
#   "medium" moderate persistence    (lag-1 autocorr ~ 0.43)
#   "high"   strong, long memory     (lag-1 autocorr ~ 0.74)
#
# Returns a T x T positive-definite correlation (Toeplitz) matrix.
build_arbitrary_serial_cov <- function(TT, strength = c("low", "medium", "high")) {
  ...
  lags <- 0:(TT - 1L)
  S    <- matrix(0, TT, TT)
  for (m in seq_along(spec$rho))
    S <- S + spec$w[m] * toeplitz(spec$rho[m]^lags)
  d <- sqrt(diag(S))                       # normalise to unit marginal variance
  S <- S / outer(d, d)
  ev <- eigen(S, symmetric = TRUE, only.values = TRUE)$values   # PD safety net
  if (min(ev) < 1e-10) {
    eg  <- eigen(S, symmetric = TRUE)
    S   <- eg$vectors %*% (pmax(eg$values, 1e-8) * t(eg$vectors))
...
generate_data <- function(beta_g_vec, r_g_vec) {
  ...
  eps     <- as.vector(t(mvrnorm(N_total, rep(0, T_total), Sigma_err)))
...
## Sweep over arbitrary serial-correlation strength {low, medium, high}. For each
## regime we rebuild the general within-unit covariance Sigma_err (used by
## generate_data) and reset the seed so the base random draws are identical across
## regimes -- only the serial-correlation structure changes.
res_by_serial        <- vector("list", length(serial_types))
names(res_by_serial) <- serial_types

for (si in seq_along(serial_types)) {
  stype     <- serial_types[si]
  Sigma_err <- build_arbitrary_serial_cov(T_total, stype)   # global; used by generate_data
  acf_imp   <- Sigma_err[1L, seq_len(min(6L, T_total))]     # implied ACF, lags 0-5
  min_ev    <- min(eigen(Sigma_err, symmetric = TRUE, only.values = TRUE)$values)
  
  set.seed(312844)                                          # same base draws across regimes
  cat(sprintf("\n################  serial = %s  ################\n", stype))
  cat(sprintf("Implied ACF (lags 0-5): %s\n",
              paste(sprintf("%.3f", acf_imp), collapse = ", ")))
  cat(sprintf("min eigenvalue of Sigma_err: %.3e\n", min_ev)); flush(con)
  res_by_serial[[si]] <- run_simulation(beta_het, r_het,
                                        sprintf("Heterogeneous (serial=%s)", stype))
}

## ---- Combined summary across regimes: estimators (rows) x serial (cols) ----
est_names  <- res_by_serial[[1L]]$summary$Estimator
serial_lab <- sprintf("serial=%s", serial_types)
bias_tab   <- data.table(Estimator = est_names)
var_tab    <- data.table(Estimator = est_names)
for (si in seq_along(serial_types)) {
  s   <- res_by_serial[[si]]$summary
  idx <- match(est_names, s$Estimator)
  bias_tab[[serial_lab[si]]] <- round(s$Bias[idx], 4)
  var_tab[[ serial_lab[si]]] <- round(s$Variance[idx], 4)
}

cat("\n==================== SUMMARY ACROSS serial-correlation regimes ====================\n")
cat(sprintf("True ATT (constant across regimes): %.4f\n", res_by_serial[[1L]]$true_att))
cat("\nBias  (estimator x serial):\n");     print(bias_tab, row.names = FALSE)
cat("\nVariance (estimator x serial):\n");  print(var_tab,  row.names = FALSE)

save(res_by_serial, bias_tab, var_tab, serial_types,
     file = "/home/rishabh.bijani_ug2023/gmm_sims_01_06/hetcov_dgp2_arbitrary_serial_sweep.RData")
...
Bias  (estimator x serial):
  Estimator serial=low serial=medium serial=high
<char>      <num>         <num>       <num>
  TWFE     4.0013        4.0046      3.9964
CS    -0.0072       -0.0076     -0.0138
SA    -0.0072       -0.0076     -0.0138
Gardner    -0.0038       -0.0052     -0.0106
Flex_TWFE    -0.0038       -0.0052     -0.0106
GMM_I    -0.0029       -0.0058     -0.0109
GMM_II    -0.0038       -0.0063     -0.0113
GMM_III    -0.0052       -0.0055     -0.0123
GLS_I    -0.0022       -0.0033     -0.0122
GLS_II    -0.0056       -0.0050     -0.0119
GLS_III    11.2189        9.6810      7.6302


Variance (estimator x serial):
  Estimator serial=low serial=medium serial=high
<char>      <num>         <num>       <num>
  TWFE     0.0107        0.0159      0.0246
CS     0.0470        0.0530      0.0520
SA     0.0470        0.0530      0.0520
Gardner     0.0164        0.0296      0.0526
Flex_TWFE     0.0164        0.0296      0.0526
GMM_I     0.0162        0.0299      0.0418
GMM_II     0.0174        0.0308      0.0434
GMM_III     0.0180        0.0321      0.0594
GLS_I     0.0315        0.0301      0.0430
GLS_II     0.0220        0.0365      0.0447
GLS_III     0.9025        1.6048      2.4109
...
Bias  (estimator x serial):
  Estimator serial=low serial=medium serial=high
<char>      <num>         <num>       <num>
  TWFE     4.0040        4.0084      4.0018
CS     0.0023        0.0046     -0.0041
SA     0.0023        0.0046     -0.0041
Gardner    -0.0006        0.0032     -0.0022
Flex_TWFE    -0.0006        0.0032     -0.0022
GMM_I    -0.0005        0.0022     -0.0011
GMM_II    -0.0004        0.0024     -0.0009
GMM_III     0.0006       -0.0006      0.0001
GLS_I    -0.0075        0.0031     -0.0014
GLS_II    -0.0002        0.0021     -0.0014
GLS_III     0.0001       -0.0010     -0.0004

Variance (estimator x serial):
  Estimator serial=low serial=medium serial=high
<char>      <num>         <num>       <num>
  TWFE     0.0022        0.0031      0.0048
CS     0.0090        0.0096      0.0102
SA     0.0090        0.0096      0.0102
Gardner     0.0033        0.0058      0.0100
Flex_TWFE     0.0033        0.0058      0.0100
GMM_I     0.0033        0.0056      0.0082
GMM_II     0.0034        0.0057      0.0083
GMM_III     0.0044        0.0079      0.0113
GLS_I     0.0254        0.0071      0.0084
GLS_II     0.0051        0.0060      0.0085
GLS_III     0.0045        0.0078      0.0112
