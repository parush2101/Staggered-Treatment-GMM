...
\item \textit{Notes:} Standard errors in parentheses.
  ...
  SA uses interaction-weighted aggregation; always-treated states coded
  as never-treated ($g = \infty$) in Panel A.
...
true effect. Gardner and Flexible TWFE, which use an imputation approach
to estimate counterfactual outcomes from pre-treatment variation, yield
positive and statistically significant estimates around $+0.020$ in both
panels.
...
The divergence between Gardner/Flexible TWFE (positive, $\approx+0.020$)
and CS (near zero) reflects a subtler difference. Both eliminate
forbidden comparisons, but they exploit the data differently. Gardner
and Flexible TWFE rely on the parallel trends assumption holding
globally---they use the pre-treatment variation of all untreated units to
estimate counterfactual trends for all treated cohorts. CS estimates
cohort-specific counterfactuals using only the cohort's own not-yet-treated
comparisons. When treatment effect heterogeneity is strong and cohort
compositions differ, the two approaches can diverge.
...
\textit{CS, Gardner, and Flexible TWFE are unbiased but differ in
efficiency.} All three eliminate forbidden comparisons and are
consistent under parallel trends. The sign disagreement between
CS (near zero) and Gardner/Flexible TWFE (positive) reflects
genuine differences in how each method constructs counterfactual
trends. The CS estimate, which makes no cross-cohort extrapolation,
may be the most conservative and credible among this group.
