###############################################################################
# Table 2 Replication with AR(1) Dependent Errors — Large N (UNBALANCED PANEL)
#
# DGP: Y_it = alpha_i + lambda_t + tau_it + epsilon_it
#   where epsilon_i ~ MVN(0, Sigma) with Sigma[t,s] = rho^|t-s|
#   UNBALANCED: Observations are randomly dropped with probability 0.10
#   (units missing at random, approximately 90% retention rate)
#
# Estimators:
#   1. TWFE                    5. GMM (A = I)
...
generate_data <- function(beta_g_vec, r_g_vec) {
  ...
  # UNBALANCED PANEL: Randomly drop observations with probability 0.10
  # (Missing at random — independent of treatment status and cohort)
  keep_idx <- rbinom(nrow(dt), size = 1, prob = 0.9) == 1
  dt <- dt[keep_idx, ]
...
cat("================================================================\n")
cat(sprintf("  UNBALANCED PANEL: N~%d (%d/cohort, 90%% retention), T=%d, rho=%.1f, %d sims\n",
...
cat("\n\n")
cat("==========================================================================\n")
cat(sprintf("  TABLE 2 (UNBALANCED): Bias and Variance — AR(1) (rho=%.1f), 90%% retention\n",
            rho))
...
save(res_hom, res_het, table2,
     ...
     file = "simulation_dependent_errors_unbalanced_results.RData")
cat("\nResults (incl. CATTs) saved to simulation_dependent_errors_unbalanced_results.RData\n")
