###############################################################################
# Table 2 Replication — Heterogeneous-Variance DGP (GMM_Eff_HetVar design)
#
# DGP: Y_it = alpha_i + lambda_t + tau_it + epsilon_it
#   where epsilon_i ~ MVN(0, Sigma_g) with Sigma_g[t,s] = sigma2_g * rho_g^|t-s|
#   Each cohort g has its own AR(1) parameter rho_g and marginal variance sigma2_g.
#   Cohorts are independent of each other (Assumption 1 holds).
#   => GMM_Eff_HetVar is the efficient estimator; GMM_Eff (pooled sigma) is not.
#
# Estimators:
#   1. TWFE
...
# Cohort-specific AR(1) parameters (order: g=10, 13, 16, 19, 22, 0=never-treated)
# Strongly varying rho and sigma2 so that GMM_Eff_HetVar gains are visible
cohort_dgp_order <- c(treatment_times, 0L)   # matches unit_cohort construction
rho_by_cohort    <- c(0.2, 0.5, 0.7, 0.9, 0.4, 0.6)   # one per cohort
sigma2_by_cohort <- c(0.5, 1.0, 2.0, 3.0, 1.5, 1.0)   # marginal variance per cohort
Sigma_by_cohort  <- lapply(seq_along(cohort_dgp_order), function(gi)
  sigma2_by_cohort[gi] * toeplitz(rho_by_cohort[gi]^(0:(T_total - 1))))
...
generate_data <- function(beta_g_vec, r_g_vec) {
  ...
  # Draw errors from each cohort's own covariance matrix (cross-cohort independent)
  ...
  for (gi in seq_along(cohort_dgp_order)) {
    ...
    eps_mat[units_g, ] <- mvrnorm(length(units_g), rep(0, T_total), Sigma_by_cohort[[gi]])
  }
...
cat(sprintf("  rho_by_cohort:   %s\n", paste(round(rho_by_cohort, 2), collapse = ", ")))
cat(sprintf("  sigma2_by_cohort: %s\n", paste(round(sigma2_by_cohort, 2), collapse = ", ")))
...
cat("\n\n")
cat("==========================================================================\n")
cat(sprintf("  TABLE 2: Bias and Variance — Heterogeneous-Variance DGP, N=%d (%d/cohort)\n",
            N_total, cohort_size))
cat(sprintf("  rho: %s | sigma2: %s\n",
            paste(round(rho_by_cohort, 2), collapse = ","),
            paste(round(sigma2_by_cohort, 2), collapse = ",")))
